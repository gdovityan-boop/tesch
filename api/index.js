...
app.post('/api/auth/login', async (req, res) => {
  ...
  console.log(`[AUTH v2.8.3] Attempt: ${email} | Mode: ${isRegister ? 'REGISTER' : 'LOGIN'}`);
  ...
  try {
    ...
    if (!isRegister) {
        // Mode: LOGIN
        if (existing.length === 0) {
            // [UPDATED ERROR MESSAGE FOR v2.8.3]
            return res.status(404).json({ error: "⛔ [v2.8.3] Аккаунт не найден. Переключитесь на вкладку 'Регистрация'." });
        }
        return res.json(existing[0]);
    } else {
        // Mode: REGISTER
        if (existing.length > 0) {
             // [UPDATED ERROR MESSAGE FOR v2.8.3]
             return res.status(409).json({ error: "⛔ [v2.8.3] Email уже занят. Переключитесь на вкладку 'Вход'." });
        }
...
app.get('/api/health', (req, res) => {
    res.json({ status: 'ok', timestamp: new Date(), server: 'Vercel Serverless Function v2.8.3' });
});
