                 ...
                 {myOrders.filter(o => o.status === OrderStatus.COMPLETED).flatMap(order => order.items).length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4">
                        {myOrders.filter(o => o.status === OrderStatus.COMPLETED).flatMap(order => order.items.map((item, idx) => (
                            <div key={`${order.id}-${idx}`} className="bg-cyber-800/40 p-4 rounded-xl border border-white/5 flex flex-col gap-4 group hover:border-cyber-primary/30 transition-all h-full">
        ...
        {activeTab === 'settings' && (
            <div className="w-full animate-in fade-in zoom-in-95 duration-300 space-y-6">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                    {/* LEFT COLUMN: PUBLIC PROFILE (2/3 width) */}
                    ...
                    {/* RIGHT COLUMN: NOTIFICATIONS & SECURITY (1/3 width, stacked) */}
                                ...
                                <div className="p-3 rounded-lg bg-black/20 border border-white/5 flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-gray-500">Active • IP: 192.168.x.x</p>
                                        </div>
                                    </div>
